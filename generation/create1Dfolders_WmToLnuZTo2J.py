...
if __name__ == "__main__":
  ...
  for param in params:
    ...
    f_launchfile = open ('launch_WmToLnuZTo2J_' + param[1] + '_LI.txt', 'w')
    ...
    f_launchfile.write ("define vl~ = ve~ vm~ vt~\n")
    f_launchfile.write ('generate p p > w- z j j QCD=0 SMHLOOP=0 NP=1 NP^2==1, w- > l- vl~, z > j j\n')
    f_launchfile.write ('output WmToLnuZTo2J_' + param[1] + '_LI')
    f_launchfile.close ()

  # generate the quadratic component folders
  for param in params:
    if param[0] not in switchOn : continue   
    f_launchfile = open ('launch_WmToLnuZTo2J_' + param[1] + '_QU.txt', 'w')
    f_launchfile.write ('import model SMEFTsim_U35_MwScheme_UFO-' + param[1] + '_massless\n')
    f_launchfile.write ("define p = g u c d s b u~ c~ d~ s~ b~\n")
    f_launchfile.write ("define j = p\n")
    f_launchfile.write ("define l+ = e+ mu+ ta+\n")
    ...
    f_launchfile.write ("define vl = ve vm vt\n")
    f_launchfile.write ("define vl~ = ve~ vm~ vt~\n")
    f_launchfile.write ('generate p p > w- z j j QCD=0 SMHLOOP=0 NP=1 NP^2==2, w- > l- vl~, z > j j\n')
    f_launchfile.write ('output WmToLnuZTo2J_' + param[1] + '_QU')
    f_launchfile.close ()

  # generate the SM component
  if (len (sys.argv) > 1):
    f_launchfile = open ('launch_WmToLnuZTo2J.txt', 'w')
    ...
    f_launchfile.write ("define vl = ve vm vt\n")
    f_launchfile.write ("define vl~ = ve~ vm~ vt~\n")
    f_launchfile.write ('generate p p > w- z j j QCD=0 SMHLOOP=0, w- > l- vl~, z > j j\n')
    f_launchfile.write ('output WmToLnuZTo2J_SM')    
    f_launchfile.close ()
