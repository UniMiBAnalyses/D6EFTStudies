...
def findXS4SM (filename):
    ...
    for idx, line in enumerate(content):
       if line.startswith("<MGGenerationInfo>"):
          nevents = int(content[idx+1].split(":")[1].strip())
    ...
    for idx, line in enumerate(content):
       if line.startswith("<event>"):
          #print(content[idx+1].split(" "))
          xs_ = content[idx+1].split(" ")[8]
          xs_err = content[idx+1].split(" ")[8]
          xs.append(float(xs_))
          if float(xs_) < 0: print(content[idx+1].split(" "))
    n = np.array(xs)
    mean_xs = np.mean(xs) 
