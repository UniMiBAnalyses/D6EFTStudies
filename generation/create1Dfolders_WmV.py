...
if __name__ == "__main__":
  ...
  for param in params:
    ...
    f_launchfile = open ('launch_emV_' + param[1] + '_LI.txt', 'w')
    ...
    f_launchfile.write ("define vl~ = ve~\n")
    f_launchfile.write ('generate p p > l- vl~ j j j j QCD=2 SMHLOOP=0 NP=1 NP^2==1\n')
    f_launchfile.write ('output emV_' + param[1] + '_LI')
    f_launchfile.close ()

  # generate the quadratic component folders
  for param in params:
    if param[0] not in switchOn : continue   
    f_launchfile = open ('launch_emV_' + param[1] + '_QU.txt', 'w')
    f_launchfile.write ('import model SMEFTsim_topU3l_MwScheme_UFO_b_massless-' + param[1] + '_massless\n')
    f_launchfile.write ("define p = g u c d s u~ c~ d~ s~ b b~\n")
    f_launchfile.write ("define j = p\n")
    f_launchfile.write ("define l+ = e+\n")
    ...
    f_launchfile.write ("define vl = ve\n")
    f_launchfile.write ("define vl~ = ve~\n")
    f_launchfile.write ('generate p p > l- vl~ j j j j QCD=2 SMHLOOP=0 NP=1 NP^2==2\n')
    f_launchfile.write ('output emV_' + param[1] + '_QU')
    f_launchfile.close ()

  # generate the SM component
  if (len (sys.argv) > 1):
    f_launchfile = open ('launch_emV_SM.txt', 'w')
    ...
    f_launchfile.write ('output emV_SM')    
    f_launchfile.close ()
