...
make_gridpack () {
      ...
      if [ -e $CARDSDIR/${name}_extramodels.dat ]; then
        ...
        do
          ...
          if [[ $model = *[!\ ]* ]]; then
            ...
            pushd SMEFTsim_A_MwScheme_UFO_int_orders
            git clone https://github.com/UniMiBAnalyses/D6EFTStudies.git
            cd D6EFTStudies
	    git checkout tesi_triennale_eft
	    mv madgraph_model/* ../
	    cd ..
            rm -rf D6EFTStudies
            popd
    ...
    #   echo "ninja = ../mgbasedir/HEPTools/lib" >> ./Cards/amcatnlo_configuration.txt
      ...
      echo "set FRBLOCK 57 2" >> makegrid.dat
      if [ -e $CARDSDIR/${name}_customizecards.dat ]; then
              cat $CARDSDIR/${name}_customizecards.dat >> makegrid.dat
              echo "" >> makegrid.dat
      fi
...
if [ -n "$5" ]
  then
    ...
    scram_arch=slc7_amd64_gcc820 #slc6_amd64_gcc481
fi
